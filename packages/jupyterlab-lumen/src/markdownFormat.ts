...
import { parseMarkdownHeading } from "lumen-kernel";
...
export const getMetadataOutlineHeadingLevel = (
  ...
  const level = (lumen as { headingLevel?: unknown }).headingLevel;
...
export const applyOutlineHeading = (
  ...
  const lumen = (cell.getMetadata("lumen") as Record<string, unknown> | null) ?? {};
  ...
  if (current === level) {
    ...
    if (Object.keys(next).length === 0) {
      cell.deleteMetadata("lumen");
    } else {
      cell.setMetadata("lumen", next);
    }

    return;
  }

  cell.setMetadata("lumen", { ...lumen, headingLevel: level });
};
