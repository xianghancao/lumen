...
const createBackgroundPreview = (
  ...
  preview.className = "jp-LumenBackgroundDropdown-preview";
  preview.dataset.lumenBackgroundPreview = value;
...
export const applyBackgroundToViewport = (
  ...
  if (background === "custom" && customColor) {
    viewport.style.setProperty("--lumen-background-custom-color", customColor);
  } else {
    viewport.style.removeProperty("--lumen-background-custom-color");
  }
};
...
export const createBackgroundToolbar = (
  ...
  const rebuildMenu = () => {
    ...
    getMindMapBackgrounds(t).forEach(({ value, label, title }) => {
      ...
      labelEl.className = "jp-LumenBackgroundDropdown-label";
    ...
    appendColorPickerSection(
      ...
      BACKGROUND_COLOR_SWATCHES,
      {
        ...
        customInputId: "jp-LumenBackgroundColorCustom-input",
      },
    );
  };
