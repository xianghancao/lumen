...
First public release of the Jupyter-native mind map stack (`lumen-kernel` + `jupyterlab-lumen`). Replaces the archived React/Tiptap prototype under `old_lumen/`.

### Added

- **Lumen Mind Map** document factory for `.ipynb` files in JupyterLab 4
- **Heading-driven outline tree** in `lumen-kernel` (`#` = root, nested `##`–`######`, content cells attach to current heading)
...
- **Notebook ↔ Lumen focus sync** — selecting a cell in the classic editor pans to the matching node
...
- **Product menu** (Lumen logo): current version, latest version check, GitHub repository link
- **Empty node insertion** via Tab (child) / Enter (sibling) with correct outline hierarchy via `metadata.lumen.headingLevel`
